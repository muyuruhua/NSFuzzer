...
VERIFY_SCRIPT="${BASE_DIR}/verify_protocol_image_version.sh"

if [[ "${STRICT_PROTOCOL_VERSION:-1}" == "1" ]]; then
  if [[ -f "${VERIFY_SCRIPT}" ]]; then
    bash "${VERIFY_SCRIPT}" "${DOCIMAGE}" || exit 1
  else
    echo "[version-check] warning: verifier not found at ${VERIFY_SCRIPT}, continue without check"
  fi
fi
...
for i in $(seq 1 $RUNS); do
  id=$(docker run -d -it -v /proc:/host/proc:ro --privileged $DOCIMAGE /bin/bash -c "cd \${WORKDIR:-/home/ubuntu/experiments} && run ${FUZZER} ${OUTDIR} '${OPTIONS}' ${TIMEOUT} ${SKIPCOUNT}")
...
for id in ${cids[@]}; do
  ...
  if docker cp ${id}:/tmp/sv_range.json ${SAVETO}/${OUTDIR}_sv_range_${index}.json > /dev/null; then
    :
  else
    printf "\n${FUZZER^^}: Warning: /tmp/sv_range.json not found in container ${id}, skipping."
  fi
  index=$((index+1))
done
